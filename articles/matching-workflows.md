...
- **Left group** (treatment, exposed, cases): $n_{L}$ units
- **Right group** (control, unexposed, controls): $n_{R}$ units

Each unit $i$ has a covariate vector
$\mathbf{x}_{i} \in {\mathbb{R}}^{p}$ describing characteristics we want
to balance (age, income, education, etc.).

**Goal:** Find one-to-one matches that minimize total distance:

$$\min\limits_{\pi}\sum\limits_{i = 1}^{n}d\left( \mathbf{x}_{i}^{L},\mathbf{x}_{\pi{(i)}}^{R} \right)$$

where $d( \cdot , \cdot )$ is a distance function (typically Euclidean
or Mahalanobis) and $\pi$ is a matching assignment.
...
$$d\left( \mathbf{x}_{i},\mathbf{x}_{j} \right) = \sqrt{\sum\limits_{k = 1}^{p}\left( x_{ik} - x_{jk} \right)^{2}}$$

**Mahalanobis distance** (accounts for correlations):
$$d\left( \mathbf{x}_{i},\mathbf{x}_{j} \right) = \sqrt{\left( \mathbf{x}_{i} - \mathbf{x}_{j} \right)^{\top}\Sigma^{- 1}\left( \mathbf{x}_{i} - \mathbf{x}_{j} \right)}$$

**Propensity score distance** (single dimension):
$$d(i,j) = \left| p_{i} - p_{j} \right|$$

where $p_{i} = P\left( \text{treatment} \mid \mathbf{x}_{i} \right)$ is
...
- Uses linear assignment algorithms ($O\left( n^{3} \right)$ complexity)
...
#> elapsed    sorted    0.084        0.0912          18.24
#> elapsed1 row_best    0.078        0.0968          19.36
#> elapsed2       pq    0.085        0.0912          18.24
```
...
$$\text{Std Diff} = \frac{{\bar{x}}_{\text{left}} - {\bar{x}}_{\text{right}}}{\sqrt{\left( s_{\text{left}}^{2} + s_{\text{right}}^{2} \right)/2}}$$
...
**Optimal matching complexity:** $O\left( n^{3} \right)$ using
Jonker-Volgenant
...
**Greedy matching complexity:** $O\left( n^{2}\log n \right)$ for
sorted, $O\left( n^{2} \right)$ for row-best
...
**Symptom**:
[`match_couples()`](https://gcol33.github.io/couplr/reference/match_couples.md)
runs for minutes or doesn’t complete. **Cause**: $O\left( n^{3} \right)$
