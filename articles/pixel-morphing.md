...
Given two sets of entities $A = \{ a_{1},\ldots,a_{n}\}$ and
$B = \{ b_{1},\ldots,b_{n}\}$, find the optimal one-to-one
correspondence by minimizing

$$\min\limits_{\pi}\sum\limits_{i = 1}^{n}c_{i,\pi{(i)}}$$

where the cost combines feature similarity and spatial proximity:

$$c_{ij} = \alpha\, d_{\text{feature}}\left( a_{i},b_{j} \right) + \beta\, d_{\text{spatial}}\left( \mathbf{x}_{i},\mathbf{x}_{j} \right).$$
...
**Weights** $\alpha,\beta \geq 0$ balance feature matching vs. spatial
coherence.
...
$$c_{ij} = \alpha\, \parallel \text{RGB}_{i}^{A} - \text{RGB}_{j}^{B} \parallel_{2} + \beta\, \parallel \left( x_{i},y_{i} \right) - \left( x_{j},y_{j} \right) \parallel_{2},$$

where color distances are normalized to
$\left\lbrack 0,\sqrt{3} \right\rbrack$ (RGB in $\lbrack 0,1\rbrack$)
and spatial distances to $\lbrack 0,1\rbrack$ using the image diagonal.
...
$$\left. \text{quantize}:{\mathbb{R}}^{d}\rightarrow\{ 1,\ldots,k\}, \right.$$
...
Form groups
$$G_{A}^{(c)} = \{ i:\text{quantize}\left( f_{i} \right) = c\}$$ and
similarly for $B$.
...
$$c\prime_{ij} = \alpha\, d\left( p_{i},p_{j} \right) + \beta\, d\left( {\bar{\mathbf{x}}}_{i},{\bar{\mathbf{x}}}_{j} \right),$$

where $p_{i}$ is the palette color and ${\bar{\mathbf{x}}}_{i}$ the
centroid position of group $i$.

4.  **Assign entities**

Every entity in $G_{A}^{(c)}$ is assigned according to the
group-to-group match.

#### Complexity Reduction

- Original: $O\left( n^{3} \right)$ for an $n \times n$ LAP  
- Quantized: $O\left( k^{3} + nk \right)$ for the $k \times k$ LAP plus
  group assignment  
- Speedup: approximately $(n/k)^{3}$

For example, with $n = 1600$ (a $40 \times 40$ image) and $k = 64$ you
get
...
Divide the domain into $m \times m$ patches (for example $m = 4$ so you
get $16$ patches). Denote the subset of entities of $A$ in patch $k$ by

$$P_{A}^{(k)} = \{ a_{i}:\mathbf{x}_{i} \in \text{Patch}_{k}\}.$$
...
Within each matched patch pair $\left( P_{A}^{(k)},P_{B}^{(l)} \right)$:

- If $|P_{A}^{(k)}| \leq \tau$ (a threshold, e.g. $\tau = 50$) solve the
  subproblem exactly.  
...
patches), the work can be made close to $O\left( n\log n \right)$ in
practice, compared to $O\left( n^{3} \right)$ for a single full LAP.
Intuitively, the LAPs near the leaves are very small, and the costly
large LAP is replaced by a series of much smaller ones.
...
of color and spatial distances, conversion between $(x,y)$ coordinates
and raster indexing, and handling remainder patches when the grid does
not divide evenly.
...
$$A\prime = \text{downsample}(A,s),\qquad B\prime = \text{downsample}(B,s).$$

Now $A\prime$ and $B\prime$ each have $n\prime = n/s^{2}$ entities.

2.  **Solve at low resolution**

Compute an exact LAP solution on the $n\prime \times n\prime$ problem:

$$\pi\prime = \arg\min\limits_{\pi\prime}\sum\limits_{i = 1}^{n\prime}c\prime_{i,\pi\prime{(i)}}.$$

3.  **Upscale assignment**

Map the low-resolution assignment back to full resolution:

$$\pi(i) = \text{upscale}\!(\pi\prime\left( \text{coarse\_index}(i) \right),s),$$
...
- Original: $O\left( n^{3} \right)$  
- Downscaled:
  $O(\left( n/s^{2} \right)^{3}) = O\left( n^{3}/s^{6} \right)$  
- Speedup: $s^{6}$
...
|----------------------|---------------------|-----------------------|--------------------------------------|
| Exact LAP            | $1 \times$          | Optimal               | $n \leq 1000$                        |
...
Approximate performance: up to about $100 \times 100$ (10 000 pixels) on
typical hardware is fine with the exact LAP.
...
where $a_{s},b_{s}$ are abundances of species $s$ in plots $a$ and $b$.
...
For large studies ($n > 1000$) a hierarchical approach by region is more
practical:
...
**Problem**: track $n$ particles between frame $t$ and $t + \Delta t$ in
experimental video.
...
$$d_{\text{spatial}}(i,j) = \parallel \mathbf{x}_{i} + \mathbf{v}_{i}\Delta t - \mathbf{x}_{j} \parallel_{2},$$

where $\mathbf{v}_{i}$ is the estimated velocity from previous frames.
...
{0,} & {{\text{if}\mspace{6mu}}\text{element}_{i} = \text{element}_{j},} \\
{\infty,} & \text{otherwise.}
\end{cases}$$
...
$\left. T:A\rightarrow B \right.$ minimizing

$$\int_{A}c\left( \mathbf{x},T(\mathbf{x}) \right)\, d\mu(\mathbf{x}).$$
...
$$\min\limits_{\gamma}\int_{A \times B}c(\mathbf{x},\mathbf{y})\, d\gamma(\mathbf{x},\mathbf{y})$$

subject to marginal constraints on $\gamma$.

### Discrete Linear Assignment

For discrete uniform distributions with $n$ points in $A$ and $B$ we
obtain exactly the linear assignment problem:

$$\min\limits_{\pi \in S_{n}}\sum\limits_{i = 1}^{n}c_{i,\pi{(i)}},$$

which is what `couplr` solves efficiently.

### Wasserstein Distance

With $c_{ij} = d\left( \mathbf{x}_{i},\mathbf{x}_{j} \right)$ (often
Euclidean distance), the optimal cost defines the $1$‑Wasserstein
distance:

$$W_{1}(\mu,\nu) = \min\limits_{\pi \in S_{n}}\sum\limits_{i = 1}^{n}c_{i,\pi{(i)}}.$$
