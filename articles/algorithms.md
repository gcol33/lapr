...
Auxiliary variables $\left( u_{i},v_{j} \right)$ associated with rows
and columns. Dual feasibility requires $u_{i} + v_{j} \leq c_{ij}$ for
all pairs. Strong duality ensures primal-dual optimality.

#### Tight Edge

An edge $(i,j)$ where the dual constraint holds with equality:
$u_{i} + v_{j} = c_{ij}$. Only tight edges can appear in optimal
solutions.
...
Associate dual variables $u_{i}$ (rows) and $v_{j}$ (columns):
...
1.  **Initialize** dual variables: $u_{i} = \min_{j}c_{ij}$, $v_{j} = 0$
...
**Complexity**: $O\left( n^{3} \right)$ expected,
...
**Complexity**: $O\left( n^{2}\log(nC)/\epsilon \right)$
...
**Complexity**: $O\left( n^{2} + nm \right)$ for $m$ edges
...
**Complexity**: $O\left( k \cdot T(n) \right)$ where $T(n)$ is single
LAP complexity
