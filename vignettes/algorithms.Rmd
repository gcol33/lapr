...
It helps to think of the LAP as a weighted bipartite graph. The following plot shows a 3x3 assignment problem where sources (left) must be matched to targets (right), with edge weights representing costs:

```{r bipartite-graph, fig.width=7, fig.height=5, fig.alt="Bipartite graph showing sources S1, S2, S3 on the left connected to targets T1, T2, T3 on the right with weighted edges representing assignment costs"}
...
The goal is to select exactly one edge per source and one per target, minimizing total weight. The optimal solution (blue edges) assigns S1 to T1, S2 to T2, and S3 to T3 with total cost 2 + 1 + 2 = 5.
...
```{r decision-flowchart, fig.width=8, fig.height=5, echo=FALSE, fig.alt="Flowchart showing algorithm selection based on cost matrix properties"}
library(ggplot2)

# Decision nodes and answers
nodes <- data.frame(
  label = c("Binary\ncosts?", "Sparsity\n> 50%?", "n > 1000?",
...
ggplot() +
  ...
  # Question nodes (diamonds)
  geom_point(data = nodes[nodes$type == "question", ],
             aes(x = x, y = y), shape = 23, size = 18,
             fill = "lightyellow", color = "darkorange", stroke = 1.5) +
  geom_text(data = nodes[nodes$type == "question", ],
            aes(x = x, y = y, label = label), size = 3, lineheight = 0.85) +
  # Answer nodes (rounded rectangles via label)
  geom_label(data = nodes[nodes$type == "answer", ],
             aes(x = x, y = y, label = label), size = 3.5, fontface = "bold",
             fill = "lightgreen", color = "darkgreen",
             label.padding = unit(0.35, "lines"), label.r = unit(0.2, "lines")) +
  # Title
