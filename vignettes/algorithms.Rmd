...
```{r bipartite-graph, fig.width=7, fig.height=5, echo=FALSE}
library(ggplot2)
...
ggplot() +
  ...
  geom_segment(data = edges_nonopt,
               ...
               color = "#aaa49b", linewidth = 0.8, alpha = 0.6) +
  # Optimal edges
  geom_segment(data = edges_opt,
               aes(x = from_x, y = from_y, xend = to_x, yend = to_y),
               color = "#93c54b", linewidth = 1.8) +
...
ggplot() +
  ...
  geom_text(data = edges, aes(x = label_x, y = label_y, label = label),
            size = 3, color = "#3e3f3a", fontface = "italic") +
